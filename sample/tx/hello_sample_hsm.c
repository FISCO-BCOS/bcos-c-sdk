...
void usage(void)
{
    ...
    printf(
        "Usage: hello_sample_hsm <config> <group_id> <hsm_key_index> <hsm_password> "
        "<hsm_lib_path>\n");
    printf("Example:\n");
    printf(
        "    ./hello_sample_hsm /sm_config_sample.ini group0 1 12345678 "
        "/usr/local/lib/libgmt0018.so\n");
    exit(0);
}
...
int main(int argc, char** argv)
{
    ...
    const char* key_index_str = argv[3];
    const char* password = argv[4];
    const char* hsm_lib_path = argv[5];

    // unsigned int key_index =
    //     (unsigned char)key_index_str[0] << CHAR_BIT | (unsigned char)key_index_str[1];
    unsigned int key_index = strtoul(key_index_str, 0L, 10);
    ...
    void* key_pair =
        bcos_sdk_use_hsm_keypair_by_keyindex_and_password(key_index, password, hsm_lib_path);
    if (!key_pair)
    {
    ...
    printf(" [HelloSample] contract_address:%s \n", contract_address);
