...
public class TestTxStructV2 {
  ...
  */
    ...
    public static void main(String[] args) throws JniException, InterruptedException, ExecutionException {
        if (args.length > 1) {
            Usage();
        }
        ...
        transactionDataStructV2.setInput(bytesInput);

        // encode TxData to hex tx data
        String txDataHex =
                TransactionStructBuilderJniObj.encodeTransactionDataStruct(transactionDataStructV2);
        // decode hex tx data to TxData
        TransactionDataV2 decodeTransactionDataStructHex =
                TransactionStructBuilderJniObj.decodeTransactionDataStructV2(txDataHex);
        ...
        Assert.assertArrayEquals(
                transactionStructV2.getDataHash(), fromHex(txDataHash));
        Assert.assertArrayEquals(
                transactionStructV2.getSignature(), fromHex(signature));

        // encode Tx to hex tx
        String txHex =
                TransactionStructBuilderJniObj.encodeTransactionStruct(transactionStructV2);
        // decode hex tx to Tx
        ...
        TransactionDataV2 transactionData = (TransactionDataV2) transactionStructV2.getTransactionData();
        TransactionDataV2 decodeTransactionData = (TransactionDataV2) decodeTransactionStructHex.getTransactionData();
        Assert.assertEquals(
                transactionData.getValue(),
                decodeTransactionData.getValue());
        Assert.assertEquals(
                transactionData.getGasLimit(),
                decodeTransactionData.getGasLimit());
        Assert.assertEquals(
                transactionData.getGasPrice(),
                decodeTransactionData.getGasPrice());
        Assert.assertEquals(
                transactionData.getMaxFeePerGas(),
                decodeTransactionData.getMaxFeePerGas());
        Assert.assertEquals(
                transactionData.getMaxPriorityFeePerGas(),
                decodeTransactionData.getMaxPriorityFeePerGas());
        Assert.assertArrayEquals(
                transactionStructV2.getDataHash(),
                decodeTransactionStructHex.getDataHash());
        Assert.assertArrayEquals(
                transactionStructV2.getSignature(),
                decodeTransactionStructHex.getSignature());
