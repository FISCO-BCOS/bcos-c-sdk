...
{
    if (transactionDataV2 == nullptr)
    {
        THROW_JNI_EXCEPTION(
            env, "exception in convert_to_tx_data_jobject_v2, transactionDataV2 is nullptr");
        return NULL;
    }
    ...
    // chain_id
    jfieldID chainIdField = env->GetFieldID(txDataClass, "chainId", "Ljava/lang/String;");
    env->SetObjectField(jTxDataObj, chainIdField, env->NewStringUTF(transactionDataV2->chain_id));
    // group_id
    jfieldID groupIdField = env->GetFieldID(txDataClass, "groupId", "Ljava/lang/String;");
    env->SetObjectField(jTxDataObj, groupIdField, env->NewStringUTF(transactionDataV2->group_id));
    // nonce
    jfieldID nonceField = env->GetFieldID(txDataClass, "nonce", "Ljava/lang/String;");
    ...
    // gas_price
    jfieldID gasPriceField = env->GetFieldID(txDataClass, "gasPrice", "Ljava/lang/String;");
    env->SetObjectField(jTxDataObj, gasPriceField, env->NewStringUTF(transactionDataV2->gas_price));
    // gas_limit
    jfieldID gasLimitField = env->GetFieldID(txDataClass, "gasLimit", "J");
...
jobject convert_to_tx_jobject_v2(JNIEnv* env, const struct bcos_sdk_c_transaction_v2* tx_struct_v2)
{
    ...
    {
        THROW_JNI_EXCEPTION(env, "exception in convert_to_tx_jobject_v2, tx_struct_v2 is nullptr");
        return NULL;
    }
    ...
    jfieldID txDataField = env->GetFieldID(
        txClass, "transactionData", "Lorg/fisco/bcos/sdk/jni/utilities/tx/TransactionDataV2;");
...
struct bcos_sdk_c_transaction_v2* convert_to_tx_struct_v2(JNIEnv* env, jobject jTxObj)
{
    if (jTxObj == NULL)
    {
        THROW_JNI_EXCEPTION(env, "exception in convert_to_tx_struct_v2, jTxObj is nullptr");
        return NULL;
    }

    jclass javaTxClass = env->GetObjectClass(jTxObj);
    jfieldID transactionDataField = env->GetFieldID(
        javaTxClass, "transactionData", "Lorg/fisco/bcos/sdk/jni/utilities/tx/TransactionDataV2;");
    jfieldID dataHashField = env->GetFieldID(
    ...
    struct bcos_sdk_c_bytes* signatureStruct = convert_to_bytes_struct_v2(env, javaSignatureObj);
    ...
    JNIEnv* env, jclass, jobject jTransactionDataObj)
{
    ...
    const char* tx_data_hex = bcos_sdk_encode_transaction_data_struct_to_hex_v2(tx_data_struct_v2);
    ...
    JNIEnv* env, jclass, jstring jTxDataHexStr)
{
    ...
    struct bcos_sdk_c_transaction_data_v2* tx_data_struct_v2 =
        bcos_sdk_decode_transaction_data_struct_from_hex_v2(tx_data_hex_str);
    if (!bcos_sdk_is_last_opr_success())
    {
        THROW_JNI_EXCEPTION(env, bcos_sdk_get_last_error_msg());
        return nullptr;
    ...
    JNIEnv* env, jclass, jobject jTransactionObj)
{
    ...
    const char* tx_hex = bcos_sdk_encode_transaction_struct_to_hex_v2(tx_struct_v2);
    if (!bcos_sdk_is_last_opr_success())
    {
        THROW_JNI_EXCEPTION(env, bcos_sdk_get_last_error_msg());
        return nullptr;
    ...
    JNIEnv* env, jclass, jstring jTxHexStr)
{
    ...
    if (jTxObj == NULL)
    {
        THROW_JNI_EXCEPTION(env, "exception in convert_to_tx_jobject_v2");
        return nullptr;
    }
